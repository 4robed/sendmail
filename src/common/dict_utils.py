...
def find_key_diff(d1, d2, path=""):
    ...
    for k in d1:
        if k not in d2:
            ok, message = False, "{} is key not in default_object, in path: {}".format(
                k, path)
        else:
            if type(d1[k]) is dict:
                if path == "":
                    ...
                    path = path + "->" + k
                ok, message = find_key_diff(d1[k], d2[k], path)
            else:
                if d1[k] != d2[k]:
                    # if you want check diff value then return here
                    # print('diff value: path={}, key={}, value: {} ~ {}'.format(path, k, d1[k], d2[k]))
                    pass
        if not ok:
            return ok, message
...
def merges_two_dict(dict_old, dict_new, path=None):
    """merges dict_new into dict_old"""
    if path is None: path = []
    for key in dict_new:
        if key in dict_old:
            if isinstance(dict_old[key], dict) and isinstance(dict_new[key], dict):
                merges_two_dict(dict_old[key], dict_new[key], path + [str(key)])
            elif dict_old[key] == dict_new[key]:
                # same value
                pass
            else:
                # if value is array then add here
                pass
        else:
